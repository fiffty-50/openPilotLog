 ...
 * \details NewFlightDialog offers two constructors, one is used to create a new flight entry from
 * scratch, where the other one is used to edit an existing entry. This entry is identified by its
 * ROW ID, which is used to retreive the entry data and pre-populate the user interface with the
 * data from the database.
 *
 * The flights table is the core of the application's database and care needs to be taken when
 * interfacing with it.
 *
 * To ensure only good data is written to the database, the ValidationState class is used. It
 * contains a QBitArray with each bit representing a mandatory data point. The array is initialized
 * to all false and progressively changed to true as entry data is validated. An entry can only be
 * submitted if all the verification bits are set.
 *
 * Inputs from the user are verified with a two-step process. The first level of verification is
 * accomplished by QRegularExpressionValidator, which limits the user to only inputting generally
 * acceptable data (like numbers for date or time, or characters for airport identifiers). If the
 * input passes this sanity check, the line edits emit the editingFinished() signal, which triggers
 * a more granular and sophisticated set of input verification, broadly based on cross-checking the
 * entered data against known good values. The ACompletionData class is used to provide QHashs of
 * known good values from the database and their respective ROW_IDs. If user-entered data has been
 * matched to a known good database value, the data is considered acceptable. This means that in
 * order to, for example, log a flight with a certain Pilot, that this pilot already has to exist in
 * the pilots table. If this is not the case, the user is prompted to add a new pilot (or aircraft)
 * to the database before proceeding. In order to make this matching process seamless for the user,
 * the completionData also contains a set of QStringLists for each of the database tables which are
 * used to create QCompleters that provide pop-up completion on the respective QLineEdits.
 *
 * Once the user is satisfied with his entries, a final set of input verification is triggered and
 * the entry is submitted to the database, see on_buttonBox_accepted() and Database::commit()
 */
class NewFlightDialog : public EntryEditDialog {
  ...
  public:
    /*!
     * \brief NewFlightDialog - Creates a NewFlightDialog that can be used to add a new flight entry
     * to the logbook
     */
    explicit NewFlightDialog(QWidget *parent = nullptr);
    /*!
     * \brief NewFlightDialog - Creates a NewFlightDialog that can be used to edit an existing entry
     * in the logbook
  ...
  private:
    ...
    /*!
     * \brief pilotNameLineEdits - Line Edits for Pilot in Command, Second in Command (Co-Pilot) and
     * Third Pilot
     */
    static const inline QList<QLineEdit *> *pilotNameLineEdits;
    /*!
     * \brief mandatoryLineEdits - Contains the Line Edits that are needed for logging a complete
     * flight from A to B. The list is ordered like the ValidationItem enum so that indexed access
     * is possible using the enum.
     */
    ...
    static const inline QHash<ValidationState::ValidationItem, QString>
        validationItemsDisplayNames = {
     ...
     * \details When a Line Edit of the mandatoryLineEdits list is edited, on editingFinished(), the
     * received input is evaluated and if considered a good input, the validation bit in
     * validationState is set.
     */
     ...
     * \details When a Line Edit of the mandatoryLineEdits list is edited, on editingFinished(), the
     * received input is evaluated and if considered a bad input, the validation bit in
     * validationState is unset.
     */
    ...
    /*!
     * \brief informUserAboutMissingItems - collect information about missing mandatory items and
