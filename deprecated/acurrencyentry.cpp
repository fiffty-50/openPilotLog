...
AEntry::AEntry(DataPosition position_) : position(position_) {}

AEntry::AEntry(RowData_T table_data) : tableData(table_data) {}
...
void AEntry::setData(RowData_T table_data) { tableData = table_data; }

void AEntry::setPosition(DataPosition position_) { position = position_; }

const DataPosition &AEntry::getPosition() const { return position; }

const RowData_T &AEntry::getData() const { return tableData; }
...
AEntry::operator QString() const
{
    ...
    for (i = tableData.constBegin(); i != tableData.constEnd(); ++i) {
        ...
        if (i.value().toString().isEmpty()) {
            out.append(QLatin1String("\t\033[m") + i.key() + spacer +
            ...
            out.append(QLatin1String("\t\033[m") + i.key() + spacer + QLatin1String("\033[35m") +
                       i.value().toString());
        ...
        item_count++;
        if (item_count % 4 == 0) out.append(QLatin1String("\n"));
    }
...
{
    return QDate::fromString(tableData.value(OPL::Db::CURRENCIES_EXPIRYDATE).toString(),
                             Qt::ISODate)
        .isValid();
}
