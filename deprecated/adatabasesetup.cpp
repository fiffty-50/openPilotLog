...
const auto createTableFlights =
    QLatin1String("CREATE TABLE flights ("
...
const auto createViewDefault = QLatin1String(
...
const auto createViewEASA = QLatin1String(
    ...
    " AS 'Type', "
    " registration AS 'Registration', "
    " (SELECT printf('%02d',(tSPSE/60))||':'||printf('%02d',(tSPSE%60)) WHERE tSPSE IS NOT NULL) "
    "AS 'SP SE', "
    " (SELECT printf('%02d',(tSPME/60))||':'||printf('%02d',(tSPME%60)) WHERE tSPME IS NOT NULL) "
    "AS 'SP ME', "
    " (SELECT printf('%02d',(tMP/60))||':'||printf('%02d',(tMP%60)) WHERE tMP IS NOT NULL) AS "
    "'MP', "
    " printf('%02d',(tblk/60))||':'||printf('%02d',(tblk%60)) AS 'Total', "
    " CASE "
    " WHEN pilot_id = 1 THEN alias "
    ...
    " ldgDay AS 'L/D', "
    " ldgNight AS 'L/N', "
    " (SELECT printf('%02d',(tNight/60))||':'||printf('%02d',(tNight%60)) WHERE tNight IS NOT "
    "NULL)  AS 'Night', "
    " (SELECT printf('%02d',(tIFR/60))||':'||printf('%02d',(tIFR%60)) WHERE tIFR IS NOT NULL)  AS "
    "'IFR', "
    " (SELECT printf('%02d',(tPIC/60))||':'||printf('%02d',(tPIC%60)) WHERE tPIC IS NOT NULL)  AS "
    "'PIC', "
    " (SELECT printf('%02d',(tSIC/60))||':'||printf('%02d',(tSIC%60)) WHERE tSIC IS NOT NULL)  AS "
    "'SIC', "
    " (SELECT printf('%02d',(tDual/60))||':'||printf('%02d',(tDual%60)) WHERE tDual IS NOT NULL)  "
    "AS 'Dual', "
    " (SELECT printf('%02d',(tFI/60))||':'||printf('%02d',(tFI%60)) WHERE tFI IS NOT NULL)  AS "
    "'FI', "
    " remarks AS 'Remarks' "
    " FROM flights "
    " INNER JOIN pilots on flights.pic = pilots.pilot_id "
...
const auto createViewQCompleter =
...
const auto createViewTotals = QLatin1String(
    "CREATE VIEW viewTotals AS "
    " SELECT "
    " printf(\"%02d\",CAST(SUM(tblk) AS INT)/60)||\":\"||printf(\"%02d\",CAST(SUM(tblk) AS "
    "INT)%60) AS \"TOTAL\", "
    " printf(\"%02d\",CAST(SUM(tSPSE) AS INT)/60)||\":\"||printf(\"%02d\",CAST(SUM(tSPSE) AS "
    "INT)%60) AS \"SP SE\", "
    " printf(\"%02d\",CAST(SUM(tSPME) AS INT)/60)||\":\"||printf(\"%02d\",CAST(SUM(tSPME) AS "
    "INT)%60) AS \"SP ME\", "
    " printf(\"%02d\",CAST(SUM(tNIGHT) AS INT)/60)||\":\"||printf(\"%02d\",CAST(SUM(tNIGHT) AS "
    "INT)%60) AS \"NIGHT\", "
    " printf(\"%02d\",CAST(SUM(tIFR) AS INT)/60)||\":\"||printf(\"%02d\",CAST(SUM(tIFR) AS "
    "INT)%60) AS \"IFR\", "
    " printf(\"%02d\",CAST(SUM(tPIC) AS INT)/60)||\":\"||printf(\"%02d\",CAST(SUM(tPIC) AS "
    "INT)%60) AS \"PIC\", "
    " printf(\"%02d\",CAST(SUM(tPICUS) AS INT)/60)||\":\"||printf(\"%02d\",CAST(SUM(tPICUS) AS "
    "INT)%60) AS \"PICUS\", "
    " printf(\"%02d\",CAST(SUM(tSIC) AS INT)/60)||\":\"||printf(\"%02d\",CAST(SUM(tSIC) AS "
    "INT)%60) AS \"SIC\", "
    " printf(\"%02d\",CAST(SUM(tDual) AS INT)/60)||\":\"||printf(\"%02d\",CAST(SUM(tDual) AS "
    "INT)%60) AS \"DUAL\", "
    " printf(\"%02d\",CAST(SUM(tFI) AS INT)/60)||\":\"||printf(\"%02d\",CAST(SUM(tFI) AS INT)%60) "
    "AS \"INSTRUCTOR\", "
    " printf(\"%02d\",CAST(SUM(tSIM) AS INT)/60)||\":\"||printf(\"%02d\",CAST(SUM(tSIM) AS "
    "INT)%60) AS \"SIMULATOR\", "
    " printf(\"%02d\",CAST(SUM(tMP) AS INT)/60)||\":\"||printf(\"%02d\",CAST(SUM(tMP) AS INT)%60) "
    "AS \"MultPilot\", "
...
const QStringList tables = {createTablePilots,   createTableTails,    createTableFlights,
                            createTableAircraft, createTableAirports, createTableCurrencies,
                            createTableChangelog};
const QStringList views  = {
    createViewDefault, createViewEASA,   createViewTails,
    createViewPilots,  createViewTotals, createViewQCompleter,
};
const QStringList userTables     = {QStringLiteral("flights"), QStringLiteral("pilots"),
                                    QStringLiteral("tails")};
const QStringList templateTables = {QStringLiteral("aircraft"), QStringLiteral("airports"),
                                    QStringLiteral("currencies"), QStringLiteral("changelog")};
...
bool ADataBaseSetup::downloadTemplates()
{
    ...
    for (const auto &table : templateTables) {
        ...
        if (downloaded_file.size() == 0) return false; // ssl/network error
    }
    return true;
}
...
bool ADataBaseSetup::backupOldData()
{
    ...
    auto date_string = ADateTime::toString(QDateTime::currentDateTime(), Opl::Datetime::Backup);
    auto backup_dir  = AStandardPaths::directory(AStandardPaths::Backup);
    QString backup_name =
        database_file.baseName() + QLatin1String("_bak_") + date_string + QLatin1String(".db");
...
bool ADataBaseSetup::importDefaultData(bool use_ressource_data)
{
    ...
    for (const auto &table_name : templateTables) {
        ...
        if (use_ressource_data) {
            ...
            data_to_commit = aReadCsv(AStandardPaths::directory(AStandardPaths::Templates)
                                          .absoluteFilePath(table_name + QLatin1String(".csv")));
...
void ADataBaseSetup::debug()
{
    ...
    for (const auto &var : types) {
        ...
        while (query.next()) {
            ...
            entries.next();
            DEB << "Element " << query.value(0).toString() << "with" << entries.value(0).toString()
                << "rows";
        }
    }
}
...
bool ADataBaseSetup::createSchemata(const QStringList &statements)
{
    ...
    for (const auto &statement : statements) {
        ...
        query.exec();
        if (!query.isActive()) {
            errors << statement.section(QLatin1Char(' '), 2, 2) + " ERROR - " +
