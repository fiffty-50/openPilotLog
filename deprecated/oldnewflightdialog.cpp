...
NewFlightDialog::NewFlightDialog(ACompletionData &completion_data, QWidget *parent)
    : QDialog(parent), ui(new Ui::NewFlight), completionData(completion_data)
{
    ui->setupUi(this);
    flightEntry = AFlightEntry();
    setup();
    if (ASettings::read(ASettings::FlightLogging::FunctionComboBox).toString() ==
        QLatin1String("SIC")) {
...
NewFlightDialog::NewFlightDialog(ACompletionData &completion_data, int row_id, QWidget *parent)
    : QDialog(parent), ui(new Ui::NewFlight), completionData(completion_data)
{
    ui->setupUi(this);
...
void NewFlightDialog::setupRawInputValidation()
{
    ...
    // create the structs holding the inititalisation data (completion lists and RXs) and store them
    // in a map
    ...
    for (const auto &line_edit : line_edits) {
        for (const auto &key : keys_list) {
            if (line_edit->objectName().contains(*key)) {
                ...
                // Set Validator
                auto validator =
                    new QRegularExpressionValidator(*init_data->validationRegEx, line_edit);
    ...
    mandatoryLineEdits = {
        ui->doftLineEdit,     ui->deptLocLineEdit, ui->destLocLineEdit, ui->tofbTimeLineEdit,
        ui->tonbTimeLineEdit, ui->picNameLineEdit, ui->acftLineEdit,
    };
    primaryTimeLineEdits = {ui->tofbTimeLineEdit, ui->tonbTimeLineEdit};
    pilotsLineEdits      = {ui->picNameLineEdit, ui->secondPilotNameLineEdit,
...
void NewFlightDialog::setupSignalsAndSlots()
{
    ...
    for (const auto &line_edit : line_edits) {
        ...
        if (line_edit->objectName().contains(LOC_LINE_EDIT_NAME)) {
            QObject::connect(line_edit, &QLineEdit::textChanged, this,
                             &NewFlightDialog::onToUpperTriggered_textChanged);
        }
        ...
        if (line_edit->objectName().contains(PILOT_LINE_EDIT_NAME)) {
            QObject::connect(line_edit, &QLineEdit::editingFinished, this,
                             &NewFlightDialog::onPilotNameLineEdit_editingFinished);
        }
        if (line_edit->objectName().contains(TIME_LINE_EDIT_NAME)) {
            QObject::connect(line_edit, &QLineEdit::editingFinished, this,
                             &NewFlightDialog::onTimeLineEdit_editingFinished);
        }
    }
#if QT_VERSION >= QT_VERSION_CHECK(5, 9, 0)
    for (const auto &line_edit : qAsConst(mandatoryLineEdits)) {
        if (line_edit->objectName().contains(QStringLiteral("doft"))) break;
        QObject::connect(line_edit->completer(),
                         QOverload<const QString &>::of(&QCompleter::highlighted), this,
                         &NewFlightDialog::onCompleter_highlighted);
        QObject::connect(line_edit->completer(),
                         QOverload<const QString &>::of(&QCompleter::activated), this,
                         &NewFlightDialog::onCompleter_activated);
    }
#endif
    QObject::connect(ui->calendarWidget, &QCalendarWidget::clicked, this,
                     &NewFlightDialog::onCalendarWidget_clicked);
}
...
void NewFlightDialog::setupUi()
{
    ...
    ui->doftLineEdit->setToolTip(
        tr("Date Format: %1").arg(ADate::getFormatString(Opl::Date::ADateFormat::ISODate)));
}
...
void NewFlightDialog::readSettings()
{
    ...
    QSettings settings;
    ui->FunctionComboBox->setCurrentText(
        ASettings::read(ASettings::FlightLogging::Function).toString());
    ui->ApproachComboBox->setCurrentIndex(
        ASettings::read(ASettings::FlightLogging::Approach).toInt());

    ASettings::read(ASettings::FlightLogging::PilotFlying).toBool()
    ...
    ui->FlightNumberLineEdit->setText(
        ASettings::read(ASettings::FlightLogging::FlightNumberPrefix).toString());
...
void NewFlightDialog::fillDeductibleData()
{
    ...
    QList<QLineEdit *> LE = {
        ui->tSPSETimeLineEdit,  ui->tSPMETimeLineEdit, ui->tMPTimeLineEdit,    ui->tIFRTimeLineEdit,
    ...
    QList<QLabel *> LB = {ui->tSPSELabel,  ui->tSPMELabel, ui->tMPLabel,    ui->tIFRLabel,
                          ui->tNIGHTLabel, ui->tPICLabel,  ui->tPICUSLabel, ui->tSICLabel,
                          ui->tDUALLabel,  ui->tFILabel};
    for (const auto &widget : LE) {
        widget->setText(QString());
    }
    for (const auto &widget : LB) {
        widget->setText(Opl::Db::NULL_TIME_hhmm);
    }
    ...
    if (locLineEditsValid()) {
        ...
        auto night_time       = ATime::qTimefromMinutes(
            ACalc::calculateNightTime(ui->deptLocLineEdit->text(), ui->destLocLineEdit->text(),
                                            dept_date_time, block_minutes, night_angle));
...
RowData_T NewFlightDialog::collectInput()
{
    ...
    // Pilots
    newData.insert(Opl::Db::FLIGHTS_PIC,
                   completionData.pilotsIdMap.key(ui->picNameLineEdit->text()));
    newData.insert(Opl::Db::FLIGHTS_SECONDPILOT,
                   completionData.pilotsIdMap.key(ui->secondPilotNameLineEdit->text()));
    newData.insert(Opl::Db::FLIGHTS_THIRDPILOT,
                   completionData.pilotsIdMap.key(ui->thirdPilotNameLineEdit->text()));

    // Extra Times
    ui->tSPSETimeLineEdit->text().isEmpty()
        ? newData.insert(Opl::Db::FLIGHTS_TSPSE, QString())
        : newData.insert(Opl::Db::FLIGHTS_TSPSE,
                         stringToMinutes(ui->tSPSETimeLineEdit->text(), flightTimeFormat));

    ui->tSPMETimeLineEdit->text().isEmpty()
        ? newData.insert(Opl::Db::FLIGHTS_TSPME, QString())
        : newData.insert(Opl::Db::FLIGHTS_TSPME,
                         stringToMinutes(ui->tSPMETimeLineEdit->text(), flightTimeFormat));
    ui->tMPTimeLineEdit->text().isEmpty()
        ? newData.insert(Opl::Db::FLIGHTS_TMP, QString())
        : newData.insert(Opl::Db::FLIGHTS_TMP,
                         stringToMinutes(ui->tMPTimeLineEdit->text(), flightTimeFormat));
    ...
    // Night
    const QString dept_date = ui->doftLineEdit->text() + 'T' + ATime::toString(tofb);
    const auto dept_date_time =
        QDateTime::fromString(dept_date, QStringLiteral("yyyy-MM-ddThh:mm"));
    const int night_angle = ASettings::read(ASettings::FlightLogging::NightAngle).toInt();
    const auto night_time = ATime::qTimefromMinutes(
        ACalc::calculateNightTime(ui->deptLocLineEdit->text(), ui->destLocLineEdit->text(),
                                  dept_date_time, block_minutes, night_angle));
    const auto night_minutes = ATime::toMinutes(night_time);
    newData.insert(Opl::Db::FLIGHTS_TNIGHT, night_minutes);

    // Function times - This is a little explicit but these are mutually exclusive so its better to
    ...
    // TO and LDG - again a bit explicit, but we  need to check for both night to day as well as day
    // to night transitions.
    ...
    if (ui->LandingCheckBox->isChecked()) {
        if (night_minutes == 0) { // all day
            ...
            const QString dest_date = ui->doftLineEdit->text() + 'T' + ATime::toString(tonb);
            const auto dest_date_time =
                QDateTime::fromString(dest_date, QStringLiteral("yyyy-MM-ddThh:mm"));
        ...
        for (const auto &leName : qAsConst(line_edits_names)) {
            if (rx.match(leName).hasMatch()) {
                ...
                if (line_edits != nullptr) {
                    DEB << "Setting " << line_edits->objectName() << " to "
                        << ATime::toString(flightEntry.getData().value(data_key).toInt(),
                                           flightTimeFormat);
                    line_edits->setText(ATime::toString(
                        flightEntry.getData().value(data_key).toInt(), flightTimeFormat));
                    line_edits_names.removeOne(leName);
                }
                break;
            }
        ...
        for (const auto &leName : qAsConst(line_edits_names)) {
            if (rx.match(leName).hasMatch()) {
                ...
                if (line_edits != nullptr) {
                    ...
                    line_edits->setText(completionData.pilotsIdMap.value(
...
bool NewFlightDialog::isLessOrEqualThanBlockTime(const QString time_string)
{
    ...
    auto block_time =
        ATime::blocktime(ATime::fromString(ui->tofbTimeLineEdit->text(), flightTimeFormat),
                         ATime::fromString(ui->tonbTimeLineEdit->text(), flightTimeFormat));
...
void NewFlightDialog::addNewTail(QLineEdit *parent_line_edit)
{
    ...
    reply =
        QMessageBox::question(this, tr("No Aircraft found"),
    ...
    if (reply == QMessageBox::Yes) {
        ...
        if (ret == QDialog::Accepted) {
            ...
            auto new_model =
                new QStringListModel(completionData.tailsList, parent_line_edit->completer());
            parent_line_edit->completer()->setModel(
                new_model); // setModel deletes old model if it has the completer as parent

            // update Line Edit
            parent_line_edit->setText(
                completionData.tailsIdMap.value(aDB->getLastEntry(ADatabaseTable::tails)));
...
void NewFlightDialog::addNewPilot(QLineEdit *parent_line_edit)
{
    ...
    if (reply == QMessageBox::Yes) {
        ...
        if (ret == QDialog::Accepted) {
            ...
            auto new_model =
                new QStringListModel(completionData.pilotList, parent_line_edit->completer());
            parent_line_edit->completer()->setModel(
                new_model); // setModel deletes old model if it has the completer as parent

            // update Line Edit
            parent_line_edit->setText(
                completionData.pilotsIdMap.value(aDB->getLastEntry(ADatabaseTable::pilots)));
            emit parent_line_edit->editingFinished();
        }
        else {
            parent_line_edit->setText(QString());
...
void NewFlightDialog::on_submitButton_clicked()
{
    ...
    if (!allMandatoryLineEditsValid()) {
        const auto display_names = QMap<int, QString>{
            {0, QObject::tr("Date of Flight")       },
            {1, QObject::tr("Departure Airport")    },
            {2, QObject::tr("Destination Airport")  },
            {3, QObject::tr("Time Off Blocks")      },
            {4, QObject::tr("Time on Blocks")       },
            {5, QObject::tr("PIC Name")             },
...
void NewFlightDialog::onGoodInputReceived(QLineEdit *line_edit)
{
    ...
    if (timeLineEditsValid()) onMandatoryLineEditsFilled();
}
...
{
    if (updateEnabled) fillDeductibleData();
}

// make sure that when using keyboard to scroll through completer sugggestions, line edit is up to
// date
...
void NewFlightDialog::onTimeLineEdit_editingFinished()
{
    ...
    if (time.isValid()) {
        ...
        if (!line_edit->text().isEmpty()) onBadInputReceived(line_edit);
    }
}
...
void NewFlightDialog::on_acftLineEdit_editingFinished()
{
    ...
    if (acft_id != 0) { // Success
        // DEB << "Mapped: " << line_edit->text() <<
        // completionData.tailsIdMap.value(line_edit->text());
    ...
    if (!line_edit->completer()->currentCompletion().isEmpty()) {
        line_edit->setText(
            line_edit->completer()->currentCompletion().split(QLatin1Char(' ')).first());
        emit line_edit->editingFinished();
        return;
    }
    ...
    if (line_edit->text() != QString()) addNewTail(line_edit);
}
...
void NewFlightDialog::on_setAsDefaultButton_clicked() { writeSettings(); }

void NewFlightDialog::on_restoreDefaultButton_clicked() { readSettings(); }
...
{
    if (timeLineEditsValid() && updateEnabled) onMandatoryLineEditsFilled();
}
...
void NewFlightDialog::on_manualEditingCheckBox_stateChanged(int arg1)
{
    ...
    QList<QLineEdit *> LE = {ui->tSPSETimeLineEdit,  ui->tSPMETimeLineEdit,  ui->tMPTimeLineEdit,
                             ui->tIFRTimeLineEdit,   ui->tNIGHTTimeLineEdit, ui->tPICTimeLineEdit,
                             ui->tPICUSTimeLineEdit, ui->tSICTimeLineEdit,   ui->tDUALTimeLineEdit,
                             ui->tFITimeLineEdit};
    switch (arg1) {
    case 0:
        ...
        updateEnabled = true;
        if (allMandatoryLineEditsValid() && updateEnabled) onMandatoryLineEditsFilled();
        break;
    case 2:
        for (const auto &le : LE) {
...
{
    if (arg1 == QLatin1String("ILS CAT III")) { // for a CAT III approach an Autoland is mandatory,
                                                // so we can preselect it.
    ...
    if (arg1 != QLatin1String("VISUAL")) ui->IfrCheckBox->setChecked(true);
}
