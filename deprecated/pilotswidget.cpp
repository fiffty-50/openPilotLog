...
PilotsWidget::PilotsWidget(QWidget *parent) : QWidget(parent), ui(new Ui::PilotsWidget)
{
    ui->setupUi(this);
...
PilotsWidget::~PilotsWidget() { delete ui; }
...
{
    QObject::connect(ui->pilotSearchLineEdit, &QLineEdit::textChanged, this,
                     &PilotsWidget::filterLineEdit_textChanged);
    QObject::connect(view->horizontalHeader(), &QHeaderView::sectionClicked, this,
                     &PilotsWidget::newSortColumnSelected);
    QObject::connect(view, &QTableView::clicked, this, &PilotsWidget::editRequested);
}
...
{
    if (signal == SettingsWidget::PilotsWidget) setupModelAndView();
}

void PilotsWidget::onPilotsWidget_databaseUpdated() { refreshView(); }
...
void PilotsWidget::filterLineEdit_textChanged(const QString &arg1)
{
    model->setFilter(QLatin1Char('\"') + ui->pilotsSearchComboBox->currentText() +
                     QLatin1String("\" LIKE '%") + arg1 + QLatin1String("%' AND ID > 1"));
}
...
void PilotsWidget::onDeleteUnsuccessful()
{
    const QList<int> foreign_key_constraints =
        DB->getForeignKeyConstraints(selectedPilots.first(), OPL::DbTable::Pilots);
    ...
    if (constrained_flights.isEmpty()) {
        ...
        for (int i = 0; i < constrained_flights.length(); i++) {
            constrained_flights_string.append(
                OPL::FlightEntry(constrained_flights[i]).getFlightSummary() +
                QStringLiteral("&nbsp;&nbsp;&nbsp;&nbsp;<br>"));
            if (i > 10) {
...
{
    if (!pilot.isValid()) return QString();
