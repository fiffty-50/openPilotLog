...
/*!
 * \brief The Row class provides an interface for retreiving and submitting entries from the
 * database.
 *
 * \details The Row class is an abstract class.
 *
 * The database holds all the data related to the logbook in different tables. Each of these tables
 * is composed of rows. Each row has different columns and each column contains the data. As such,
 * an entry can be thought of as a row in the database. The row class encapsulates the data
 * contained in each row.
 *
 * A Row is uniquely identified by its position in the database, consisting of the table name
 * (QString) and the row id (int). A new entry, which is not yet in the database has the row id 0.
 * if a new row object is created setData() has to be called before submitting the entry to the
 * database.
 *
 * The Row Object holds all the necessary information the Database class needs to commit (create or
 * update) it. The Identifying information can be accessed with getRowId() and getTable() or
 * getTableName().
 *
...
class Row {
  ...
  public:
     ...
     * \details The row data is a QHash<QString, QVariant> where the sql column name is the key and
    ...
    /*!
     * \brief returns a string representation of the entries position in the database (Table and Row
     * ID)
     */
    ...
    /*!
     * \brief A Row entry is valid if its table and row are specified and if it contains
