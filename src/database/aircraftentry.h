...
class AircraftEntry : public Row {
  ...
  public:
    ...
    enum class EngineType { UNPOWERED, PISTON, TURBOPROP, JET, ELECTRIC, OTHER };
    bool setEngineType(EngineType type);
    EngineType getEngineType() const;

    enum class AircraftClass { AEROPLANE, ROTORCRAFT, SAILPLANE, BALLOON, AIRSHIP, OTHER };
    bool setClass(AircraftClass cls);
    AircraftClass getClass() const;

    enum class AircraftSubClass { LAND, SEA };
    bool setSubClass(AircraftSubClass subCls);
    AircraftSubClass getSubClass() const;

    enum class WakeCategory { LIGHT, MEDIUM, HEAVY, SUPER };
  ...
  private:
    const static inline QList<QString> FIELDS = {
        MAKE,  MODEL,     VARIANT,        ICAO_DESIGNATOR, ENGINE_TYPE, IS_MULTI_ENGINE,
        CLASS, SUB_CLASS, IS_MULTI_PILOT, WAKE_CATEGORY,   TYPE_RATING, REMARKS};

    const static inline QList<QString> ENGINE_TYPES = {
        QStringLiteral("UNPOWERED"), QStringLiteral("PISTON"),   QStringLiteral("TURBOPROP"),
        QStringLiteral("JET"),       QStringLiteral("ELECTRIC"), QStringLiteral("OTHER"),
    };

    static QString toString(EngineType type)
    {
        switch (type) {
        case EngineType::UNPOWERED:
            return ENGINE_TYPES[0];
        case EngineType::PISTON:
            return ENGINE_TYPES[1];
        case EngineType::TURBOPROP:
            return ENGINE_TYPES[2];
        case EngineType::JET:
            return ENGINE_TYPES[3];
        case EngineType::ELECTRIC:
            return ENGINE_TYPES[4];
        case EngineType::OTHER:
            return ENGINE_TYPES[5];
        default:
            return {};
        }
    }
    ...
    const static inline QList<QString> AIRCRAFT_CLASSES = {
        QStringLiteral("AEROPLANE"), QStringLiteral("ROTORCRAFT"), QStringLiteral("SAILPLANE"),
        QStringLiteral("BALLOON"),   QStringLiteral("AIRSHIP"),    QStringLiteral("OTHER"),
    };

    static QString toString(AircraftClass cls)
    {
        switch (cls) {
        case AircraftClass::AEROPLANE:
            return AIRCRAFT_CLASSES[0];
        case AircraftClass::ROTORCRAFT:
            return AIRCRAFT_CLASSES[1];
        case AircraftClass::SAILPLANE:
            return AIRCRAFT_CLASSES[2];
        case AircraftClass::BALLOON:
            return AIRCRAFT_CLASSES[3];
        case AircraftClass::AIRSHIP:
            return AIRCRAFT_CLASSES[4];
        case AircraftClass::OTHER:
            return AIRCRAFT_CLASSES[5];
        default:
            return {};
        }
    }
    ...
    {
        switch (subCls) {
        case AircraftSubClass::LAND:
            return AIRCRAFT_SUBCLASSES[0];
        case AircraftSubClass::SEA:
            return AIRCRAFT_SUBCLASSES[1];
        default:
            return {};
        }
    }
    ...
    {
        switch (category) {
        case WakeCategory::LIGHT:
            return WAKE_CATEGORIES[0];
        case WakeCategory::MEDIUM:
            return WAKE_CATEGORIES[1];
        case WakeCategory::HEAVY:
            return WAKE_CATEGORIES[2];
        case WakeCategory::SUPER:
            return WAKE_CATEGORIES[3];
        default:
            return {};
        }
    }
