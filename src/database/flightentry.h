 ...
 *
 * The FlightEntryParser class provides convenience functions to bridge user input to database
 * format, so direct interaction with this class should not be required for most ui-facing tasks.
 */
class FlightEntry : public Row {
  ...
  private:
    ...
    static inline const QList<QString> mandatoryFields = {
        FlightEntry::DOFT, FlightEntry::DEPT, FlightEntry::DEST, FlightEntry::TOFB,
        FlightEntry::TONB, FlightEntry::TBLK, FlightEntry::PIC,  FlightEntry::ACFT};
    static inline const QList<QString> FIELDS = {
        ROWID,    DOFT,     DEPT,        DEST,       TOFB,         TONB,         PIC,     ACFT,
        TBLK,     TSPSE,    TSPME,       TMP,        TNIGHT,       TIFR,         TPIC,    TPICUS,
        TSIC,     TDUAL,    TFI,         TSIM,       PILOTFLYING,  TODAY,        TONIGHT, LDGDAY,
        LDGNIGHT, AUTOLAND, SECONDPILOT, THIRDPILOT, APPROACHTYPE, FLIGHTNUMBER, REMARKS,
    };
};
} // namespace OPL
#endif // FLIGHTENTRY_H
