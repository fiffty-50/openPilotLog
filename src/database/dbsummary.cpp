...
const QMap<DbSummaryKey, QString> DbSummary::databaseSummary(const QString &db_path)
{
    ...
    { // scope for a temporary database connection, ensures proper cleanup when removeDatabase() is
      // called.
        // DEB << "Adding temporary connection to database:" << db_path;
        QSqlDatabase temp_database = QSqlDatabase::addDatabase(
            SQLITE_DRIVER, connection_name); // Don't use default connection
        temp_database.setDatabaseName(db_path);
        if (!temp_database.open()) return {};
    ...
    QSqlDatabase::removeDatabase(connection_name); // cleanly removes temp connection without leaks
                                                   // since query+db are out of scope
...
const QString DbSummary::summaryString(const QString &db_path)
{
    ...
    out.append(
        tr("<tr><td>Total Time: </td><td>%1</td>").arg(summary_map[DbSummaryKey::total_time]));
    out.append(
        tr("<tr><td>Last Flight: </td><td>%1</td>").arg(summary_map[DbSummaryKey::last_flight]));
    out.append(tr("<tr><td>Number of flights: </td><td>%1</td>")
                   .arg(summary_map[DbSummaryKey::total_flights]));
    out.append(tr("<tr><td>Number of aircraft: </td><td>%1</td>")
                   .arg(summary_map[DbSummaryKey::total_tails]));
    out.append(tr("<tr><td>Number of Pilots: </td><td>%1</td>")
                   .arg(summary_map[DbSummaryKey::total_pilots]));
    out.append("</table>");
