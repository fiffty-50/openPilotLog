...
FlightSegmentEditorWidget::FlightSegmentEditorWidget(QWidget *parent)
    : QWidget(parent),
      ...
      startLabel(new QLabel(tr("Start"), this)),
      endLabel(new QLabel(tr("End"), this)),
      isIfrLabel(new QLabel(tr("IFR"), this)),
      isSimIfrLabel(new QLabel(tr("Sim IFR"), this)),
      ...
      pilotFunctionLabel(new QLabel(tr("Function"), this)),
      m_currentRow(0),
...
void FlightSegmentEditorWidget::loadSegments(const QList<OPL::FlightSegmentEntry> &segments)
{
    ...
    for (int i = segments.size() - 1; i >= 0; --i)
    {
        ...
        row.startEdit->setTime(
            QTime::fromMSecsSinceStartOfDay(entry.getStartMs()));
        row.endEdit->setTime(
            QTime::fromMSecsSinceStartOfDay(entry.getEndMs()));
