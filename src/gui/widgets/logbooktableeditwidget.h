 ...
 * instead of a table, it uses a [view](https://sqlite.org/lang_createview.html) as the source of
 * the display model.
 *
 * The user can select a view from a list of available views in the SettingsWidget.
 *
 * Some of the selectable views aggregate data from the flights and simulators table. In those
 * views, flight entries have positive row id's whereas the simulators have inverse (negative) row
 * id's. This necessitates checking the row id's value before deciding on the apropriate
 * EntryEditDialog.
 */
class LogbookTableEditWidget : public TableEditWidget {
  ...
  private:
    ...
    /*!
     * \brief Set up the QStyledItemDelegate instances that transform the database values to
  ...
  public slots:
     ...
     * \details The Primary Entry handled by the LogbookTableEditWidget are Flights, which are
     * stored in the flights table. However, this widget also handles adding simulator tables which
     * are stored in the simulators table.
     */
