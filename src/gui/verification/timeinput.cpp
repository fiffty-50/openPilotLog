 ...
 * \details If the user input is not a valid time, this function tries to fix it. Accepted Inputs
 * are hh:mm, h:mm, hhmm or hmm. Returns "hh:mm" or an empty string if the resulting time is
 * invalid.
 */
...
{
    if (input == QString()) return QStringLiteral("00:00");
    // try inserting a ':' for hhmm inputs
    QString fixed = input;
    if (input.contains(':')) { // contains seperator
        if (input.length() == 4) fixed.prepend(QLatin1Char('0'));
    }
