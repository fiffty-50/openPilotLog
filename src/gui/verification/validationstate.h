...
/*!
 * \brief Holds information about whether mandatory components of a new flight entry have been
 * validated.
 * \details The ValidationState class encapsulates a QBitArray that has a bit set (or unset)
 * depending on wether the input for the associated index has been verified. The indexes correspond
 * to the mandatory items enumerated in the ValidationItem enum. It is used by the NewFlightDialog
 * to keep track of validation states.
 */
class ValidationState {
  public:
    /*!
     * \brief ValidationState - The default ValidationState object is initialised with all values
     * false
     */
     ...
     * Date of Flight, Departure, Destination, Time Off Blocks, Time On Blocks, Pilot in Command,
    ...
    {
        QStringList ret;
        for (auto it = validationItemsDisplayNames.constBegin();
             it != validationItemsDisplayNames.constEnd(); ++it) {
            if (!validAt(it.key())) {
                ret.append(it.value());
            }
        }
