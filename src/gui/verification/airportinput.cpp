...
bool AirportInput::isValid() const { return DBCache->getAirportsMapICAO().key(input) != 0; }
...
QString AirportInput::fixup() const
{
    ...
    if (input.length() == 3) {
        ...
        if (id != 0) fixed = DBCache->getAirportsMapICAO().value(id);
    }
    else {
        // input could be lower case
        int id = DBCache->getAirportsMapICAO().key(input.toUpper());
        if (id != 0) fixed = input.toUpper();
    }
    return fixed;
}
