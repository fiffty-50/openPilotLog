...
ExportToCsvDialog::ExportToCsvDialog(QWidget *parent)
    : QDialog(parent), ui(new Ui::ExportToCsvDialog)
{
    ui->setupUi(this);
    init();
}

ExportToCsvDialog::~ExportToCsvDialog() { delete ui; }

void ExportToCsvDialog::on_exportPushButton_clicked()
{
    selectRows();
    // File Dialog where to save
    QString filePath = QDir::toNativeSeparators(QFileDialog::getSaveFileName(
        this, tr("Select Location"),
        QStandardPaths::writableLocation(QStandardPaths::DesktopLocation),
        QStringLiteral("*.csv")));
    if (filePath.isEmpty()) return; // user has cancelled file dialog

    if (!filePath.endsWith(QStringLiteral(".csv"))) filePath += ".csv";
    DEB << filePath;
...
void ExportToCsvDialog::selectRows()
{
    ...
    if (ui->viewComboBox->currentIndex() < 4)
        model->setTable(
            OPL::GLOBALS->getLogbookViewName(OPL::LogbookView(ui->viewComboBox->currentIndex())));
    else
