...
QDate DateTime::parseInput(QString &user_input, OPL::DateFormat format)
{
    ...
    if (return_date.isValid()) return return_date;
...
void DateTime::tryToFix(QString &user_input, OPL::DateFormat format)
{
    ...
    if (user_input.length() == 8) padCentury(user_input, format);
}
...
void DateTime::padZeroes(QString &user_input)
{
    ...
    if (match.hasMatch()) user_input.insert(match.capturedStart(), QLatin1Char('0'));

    match = unpadded_middle.match(user_input);
    if (match.hasMatch()) user_input.insert(match.capturedStart() + 1, QLatin1Char('0'));
...
{
    if (user_input.contains(QLatin1Char('.'))) return true;
    if (user_input.contains(QLatin1Char('-'))) return true;
    if (user_input.contains(QLatin1Char('/'))) return true;
...
const QStringList &DateTime::getDisplayNames() { return DISPLAY_NAMES; }
...
const QString DateTime::currentDate() { return QDate::currentDate().toString(Qt::ISODate); }
