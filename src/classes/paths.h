 ...
 * for example `/home/user/.local/share/opl` on linux or `/Users/user/Library/Application
 * Support/opl` on macos.
 *
 * OPL writes into a singular directory, with several subdirectories, which are enumerated in
 * Directories.
 */
class Paths {
  public:
    ...
    /*!
     * \brief the base Path of the Application Directory. Evaluates to an XDG writable App Data
     * location depending on the OS
     */
    static const inline QString basePath =
        QStandardPaths::writableLocation(QStandardPaths::AppLocalDataLocation) +
        QDir::toNativeSeparators("/") + ORGNAME + QDir::toNativeSeparators("/");
};
