...
typedef struct {
    ...
    double azimuth;   // topocentric azimuth angle (eastward from north) [for navigators and solar
                      // radiation]
...
//-------------- Utility functions for other applications (such as NLR's SAMPA) --------------
...
void right_ascension_parallax_and_topocentric_dec(double latitude, double elevation, double xi,
                                                  double h, double delta, double *delta_alpha,
                                                  double *delta_prime);
...
double atmospheric_refraction_correction(double pressure, double temperature, double atmos_refract,
                                         double e0);
