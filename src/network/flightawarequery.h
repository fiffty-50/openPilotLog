 ...
 *
 * When the Flight Data is ready to be read (i.e. network request finished and data parsed),
 * readyToRead() is emitted and the parsed data can be retreived by calling flightList(). A list is
 * used because some flights are multi-sector flights that share a common flight number.
 */
class FlightAwareQuery : public QObject {
    ...
    const static inline QString QUERY_BASE_URL =
        QStringLiteral("https://aeroapi.flightaware.com/aeroapi/flights/");
