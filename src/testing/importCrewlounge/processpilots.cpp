...
void ProcessPilots::parseRawData()
{
    ...
    const QVector<QVector<int>> pilot_cols = {cols_pilot1, cols_pilot2, cols_pilot3};

    QVector<QStringList> unique_pilots;
    QStringList pilot_data = {QString(), QStringLiteral("SELF"), QString(), QString()};
    ...
    for (const auto &row : std::as_const(rawData)) {
        for (const auto &col_array : pilot_cols) {
            ...
            if (!unique_pilots.contains(pilot_data) &&
                !pilot_data.contains(QLatin1String("SELF"))) {
                unique_pilots.append(pilot_data);
                processedPilotsIds.insert(pilot_data[1], unique_pilot_id);
                rawPilotsAndIds.append({pilot_data, unique_pilot_id});
                unique_pilot_id++;
...
QHash<QString, int> ProcessPilots::getProcessedPilotsIds() const { return processedPilotsIds; }
